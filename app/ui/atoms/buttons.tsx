...
export function Button({ children, className, ...props }: ButtonProps) {
  if ("href" in props) {
    return (
      <Link
        className={twMerge(
          "bg-indigo-500 text-white p-2 px-4 rounded-md m-2 hover:shadow-lg hover:shadow-slate-900 hover:scale-[1.02] hover:brightness-110 shadow-slate-800 transition-all shadow-md",
          className,
        )}
        {...props}
      >
